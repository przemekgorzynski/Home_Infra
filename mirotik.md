...
`vpn.homebay.dev` resolves to WAN IP via the existing `*.homebay.dev` Cloudflare DDNS — no extra DNS config needed.
No DST-NAT rule needed — WireGuard listens on the router itself, not an internal host.

### Setup
```routeros
# Create interface — router auto-generates private key
/interface wireguard
add name=wg0 listen-port=51820 comment="WireGuard VPN"

# Assign IP — VPN clients land in Home subnet, router side gets .241
/ip address
add address=192.168.10.241/24 interface=wg0

# Add phone peer — router auto-generates peer keypair, phone gets .242
/interface wireguard peers
add interface=wg0 allowed-address=192.168.10.242/32 comment="Phone"

# Allow WireGuard from internet
/ip firewall filter
add chain=input in-interface-list=WAN protocol=udp dst-port=51820 action=accept comment="INPUT: WireGuard"

# Allow VPN clients to reach Home VLAN
/ip firewall filter
add chain=forward in-interface=wg0 out-interface=vlan10 action=accept comment="FORWARD: wg0 → HOME"
```

### Export QR code to phone
Open WireGuard app → scan this QR code (contains all config including keys):
```routeros
/interface wireguard peers export-qr-code [find comment="Phone"]
```

Phone config generated by the QR:
- **Endpoint:** `vpn.homebay.dev:51820`
- **DNS:** `192.168.10.1` (router — resolves internal hostnames)
- **AllowedIPs:** `192.168.10.0/24` (split tunnel — only home traffic goes through VPN)

### Add more devices
```routeros
/interface wireguard peers
add interface=wg0 allowed-address=192.168.10.243/32 comment="Laptop"

/interface wireguard peers export-qr-code [find comment="Laptop"]
```
